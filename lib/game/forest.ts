...
export function generateChunk(chunkX: number, chunkY: number, config: GameConfig, worldSeed: number = 0): Chunk {
  ...
  while (trees.length < treeCount && attempts < maxAttempts) {
    ...
    if (typeRoll < t0) {
      ...
      type = TreeType.CrystalTree;      // ~0.1% (10x buff) - not within 3 chunks of spawn
    } else if (typeRoll < 0.99999 + rareBoost * 0.15 && chunkDistFromSpawn > 5) {
      type = TreeType.VoidTree;         // ~0.01% (10x buff) - not within 5 chunks of spawn
    } else if (typeRoll < 0.999999 + rareBoost * 0.04 && chunkDistFromSpawn > 10) {
      type = TreeType.CosmicTree;       // ~0.001% (10x buff) - not within 10 chunks of spawn
    } else if (typeRoll < 0.9999999 + rareBoost * 0.01 && chunkDistFromSpawn > 25) {
      type = TreeType.DivineTree;       // ~0.0001% (10x buff) - not within 25 chunks of spawn
    } else if (typeRoll >= 0.9999999 && chunkDistFromSpawn > 50) {
      type = TreeType.WorldTree;        // ~0.00001% (10x buff) - not within 50 chunks of spawn - LEGENDARY!
    } else {
      // Fallback to MagicTree if legendary tree can't spawn due to distance or roll not high enough
